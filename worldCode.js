...
function updateUi(id) {
  api.setClientOptions(id, {
    RightInfoText: styTxt(
      ...
      } \n <s 10px>XP: ${api.getMoonstoneChestItemSlot(id,3).attributes.customDisplayName} / ${100*api.getMoonstoneChestItemSlot(id,2).attributes.customDisplayName}\nv1.1`
    ),
  });
...
function levelUp(id) {
  ...
  if (newXp > 100 * currentLvl) {
    ...
    api.sendTopRightHelper(id,'trophy', "You leveled up to level " + Number(String(currentLvl + 1)) + "!",{});
    updateUi(id);
    return;
  }
  api.setMoonstoneChestItemSlot(id, 3, "Messy Stone", 1, {
...
function mSBR(id,x,y,z){
   let currentLvl = api.getMoonstoneChestItemSlot(id, 2).attributes
    .customDisplayName;
    	...
    	let chances =Math.random()*currentLvl
   	
    	let blocks = {'Messy Stone':6*currentLvl,'Coal Ore':0.5*currentLvl,'Iron Ore':0.3*currentLvl,'Gold Ore':0.1*currentLvl,'Diamond Ore':0.05*currentLvl,'Moonstone Ore':0.01*currentLvl}
